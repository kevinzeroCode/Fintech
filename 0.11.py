...
LightGBM 強化版（速度友善）— 在你的baseline上加分但不做重度圖計算
輸出：submission_lightgbm.csv
...
def entropy_from_cols(df: pd.DataFrame, prefix: str, name: str) -> pd.Series:
...
def build_features(txn_path: str, chunksize: int = 1_000_000) -> pd.DataFrame:
    ...
    for chunk in pd.read_csv(txn_path, dtype=dtypes, chunksize=chunksize):
        ...
        g_out = pd.DataFrame({
            ...
            "is_large": is_large,
            "is_self":  is_self,
            "t_m": t_morn, "t_n": t_noon, "t_e": t_even
            ...
            out_morn_sum=("t_m", "sum"),
            out_noon_sum=("t_n", "sum"),
            out_even_sum=("t_e", "sum"),
    ...
    out_df = pd.concat(out_aggs).groupby(level=0).agg({
        ...
        "out_self_sum": "sum", "out_morn_sum": "sum", "out_noon_sum": "sum", "out_even_sum": "sum",
    ...
    feat["out_morn_ratio"]  = feat["out_morn_sum"] / feat["out_cnt"].replace(0, np.nan)
    feat["out_noon_ratio"]  = feat["out_noon_sum"] / feat["out_cnt"].replace(0, np.nan)
    feat["out_even_ratio"]  = feat["out_even_sum"] / feat["out_cnt"].replace(0, np.nan)
    feat["in_morn_ratio"]   = feat["in_morn_sum"]  / feat["in_cnt"].replace(0, np.nan)
    feat["in_noon_ratio"]   = feat["in_noon_sum"]  / feat["in_cnt"].replace(0, np.nan)
    feat["in_even_ratio"]   = feat["in_even_sum"]  / feat["in_cnt"].replace(0, np.nan)

    # 近因趨勢（last7 / last30）
    for base in ["out_day_cnt", "out_day_amt", "in_day_cnt", "in_day_amt"]:
        c7, c30 = f"{base}_last7", f"{base}_last30"
        if c7 in feat.columns and c30 in feat.columns:
            feat[base + "_trend7_30"] = feat[c7] / (feat[c30] + 1e-6)

    # 統合/方向性
    feat["tot_cnt"]     = feat["out_cnt"] + feat["in_cnt"]
    feat["tot_amt_sum"] = feat["out_amt_sum"] + feat["in_amt_sum"]
    feat["net_cnt"]     = feat["in_cnt"] - feat["out_cnt"]
    feat["net_amt_sum"] = feat["in_amt_sum"] - feat["out_amt_sum"]
...
def train_and_predict(feat: pd.DataFrame,
                      ...
                      neg_pos_ratio: int = 25,
                      seeds: List[int] = [42, 2025, 7]) -> Tuple[np.ndarray, float]:
    ...
    for seed in seeds:
        ...
        model = lgb.train(params, dtrain, num_boost_round=2500, valid_sets=[dvalid], callbacks=callbacks)
...
# ============= Main =============
def main():
    print("Building features (speed-friendly)...")
    ...
    te_label, threshold = train_and_predict(
        ...
        neg_pos_ratio=25,                 # 你可試 20/25/30 做小範圍掃描
        seeds=[42, 2025, 7]               # 3 顆種子，折衷時間/穩定度
    )
